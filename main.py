...
MESSAGE_CACHE = {}    # msg_id -> restore snapshot
...
async def webhook(req: Request):
    ...
    if not message or message["from"]["id"] != ADMIN_ID:
        return {"ok": True}
    ...
    media = message.get("document") or message.get("video") \
            or message.get("audio") or message.get("voice")
    if not media:
        tg("sendMessage", {"chat_id": chat_id, "text": HINT_MSG})
        return {"ok": True}
...
def process_file(chat_id, msg_id, file_id, name, size):
    try:
        ...
        with open(name, "rb") as f:
            while True:
                if UPLOADS.get(msg_id):
                    raise Exception("UPLOAD_CANCELLED")
                chunk = f.read(1024 * 1024)
                if not chunk:
                    break
                s3.put_object(Bucket=R2_BUCKET, Key=name, Body=chunk)
